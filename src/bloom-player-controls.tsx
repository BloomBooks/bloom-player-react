...
// This logic is not straigthforward...
...
let isInternallyPaused: boolean = false;
...
> = props => {
    ...
    setExternalControlCallback(data => {
        if (data.pause) {
            ...
            setPaused(true);
            if (!wasPaused) {
                isInternallyPaused = false;
            }
        } else if (data.resume && !isInternallyPaused) {
            setPaused(false);
        } else if (data.play) {
            setPaused(false);
        }

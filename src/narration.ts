...
export default class Narration {
    ...
    private playeEndedFuction = () => {
        this.playEnded();
    };

    private getPlayer(): HTMLMediaElement {
        const audio = this.getAudio("bloom-audio-player", audio => {});
        // We used to do this in the init call, but sometimes didn't get the function called.
        ...
        audio.addEventListener("ended", this.playeEndedFuction);
        audio.addEventListener("error", this.playeEndedFuction);
        return audio;
    }

...
export class BloomPlayerCore extends React.Component<IProps, IState> {
    ...
    private displayLanguageCode: string = "";
    ...
    public componentDidUpdate(prevProps: IProps) {
        ...
        if (prevProps.activeLanguageCode !== this.props.activeLanguageCode) {
            // If we haven't set the displayLanguageCode yet, that means we're loading a new book.
            // When the activeLanguageCode is set by BloomPlayerControls, set displayLanguageCode to match.
            if (this.displayLanguageCode === "") {
                this.displayLanguageCode = this.props.activeLanguageCode;
                // We assume we don't need to update visibility of divs initially. Whatever is visible
                // should be as we want to see it normally (for the first listed language
                // in meta.json "language-display-names").
                // Resist the inclination to return here. There may be other props/state that have changed.
            } else {
        ...
        if (newSourceUrl && newSourceUrl !== this.sourceUrl) {
            // We're changing books; reset several variables including displayLanguageCode,
        ...
        if (this.swiperInstance) {
            ...
            window.setTimeout(() => {
                this.swiperInstance.lazy.load(); // Otherwise all our pictures disappear when changing language!
            }, 10);
        }
    }
    ...
    private finishUp(isNewBook: boolean = true) {
        ...
        // change it when it is done. Also, it now sets displayLanguageCode.
        this.setState({
            pages: swiperContent
        });
    ...
    private updateDivVisibilityByLangCode(): void {
        ...
        for (let iedit = 0; iedit < editableDivs.snapshotLength; iedit++) {
            ...
            // We don't want to mess with what's visible for titles

    ...
    public setupAnimation(page: HTMLElement, beforeVisible: boolean): void {
        ...
        if (!hidePageDiv || !hidePageDiv.classList.contains("hidePage")) {
            ...
            if (img || styleData) {
                ...
                if (imageSrc) {
                    image.src = imageSrc;
                    // Hide the image source from the original image container div.  Note that this div may
                    // still have an image description div with text and audio inside it.
                    if (styleData) {
                        animatableImageContainer.removeAttribute("style");
